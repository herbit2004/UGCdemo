...
## 项目介绍
- 本项目为 2025 年字节跳动工程训练营客户端方向（TikTok Global Live-社交团队）大作业项目。
...
- 必要要求（页面与能力）：
  ...
  - 出错重试：已完成（入口与状态位已设计并预留；当前以刷新与空态提示为主，继续深入需接入真实错误时展示重试）
  - 详情页：已完成（顶部栏含作者头像/昵称，内容区含图片/标题/正文，评论区含头像/昵称/评论内容）
  - 发布页：已完成（相册图片选择、标题/正文编辑、发布后返回并在列表插入所发帖子）
- 拓展功能：
  - 搜索：笔记与用户搜索，支持最近/最多喜欢/最近评论排序与分页（用户搜索当前一次性结果）
...
## 后端设计图（数据库结构 ER 图）
![readme_img/backend_flow.png](readme_img/backend_flow.png)

## 前端设计图（页面关系与导航）
![readme_img/app_flow.png](readme_img/app_flow.png)

## 前后端交互图（关键流程 Sequence）
![D:\Users\hebit\AndroidStudioProjects\UGCdemo\readme_img\Sequence_flow.png](readme_img/Sequence_flow.png)
